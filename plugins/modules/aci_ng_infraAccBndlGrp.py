...
DOCUMENTATION = r'''
---
module: auto_infraAccBndlGrp 
short_description: Manage PC/VPC Interface Policy Group (infra:AccBndlGrp)
description:
- bundled ports object
notes:
- More information about the internal APIC class B(infra:AccBndlGrp) from
...
options: 
  ...
  descr:
    description:
    - bundled ports group description 
  lagT:
    description:
    - bundled ports grp link aggregation type 
    choices: [ fc-link, link, node, not-aggregated ] 
  name:
    description:
    - bundled ports group name 
    aliases: [ pc/vpc_interface_policy_group ] 
  nameAlias:
    description:
    - Mo doc not defined in techpub!!! 
  ownerKey:
...
def main():
    ...
    argument_spec.update({ 
        ...
        'descr': dict(type='str',),
        'lagT': dict(type='str', choices=['fc-link', 'link', 'node', 'not-aggregated'], ),
        'name': dict(type='str', aliases=['pc/vpc_interface_policy_group']),
        ...
        'relation_infra_rs_acc_bndl_grp_to_aggr_if': dict(type='list'),
        ...
        'relation_infra_rs_lacp_pol': dict(type='str'),
    ...
    relation_infrarsaccbndlgrptoaggrif = module.params['relation_infra_rs_acc_bndl_grp_to_aggr_if']
    ...
    relation_infrarslacppol = module.params['relation_infra_rs_lacp_pol']
    ...
    if relation_infrarsaccbndlgrptoaggrif:
        for relation_param in relation_infrarsaccbndlgrptoaggrif:
            child_configs.append({'infraRsAccBndlGrpToAggrIf': {'attributes': {'tDn': relation_param}}})
    ...
    if relation_infrarslacppol:
        child_configs.append({'infraRsLacpPol': {'attributes': {'tnLacpLagPolName': relation_infrarslacppol}}})
    ...
    aci.construct_url(
        root_class={
            'aci_class': 'infraAccBndlGrp',
            'aci_rn': 'infra/funcprof/accbundle-{}'.format(name),
            'target_filter': 'eq(infraAccBndlGrp.name, "{}")'.format(name),
            'module_object': name
        }, 
        
        child_classes=['infraRsSpanVSrcGrp','infraRsAccBndlGrpToAggrIf','infraRsStormctrlIfPol','infraRsLldpIfPol','infraRsMacsecIfPol','infraRsQosDppIfPol','infraRsHIfPol','infraRsNetflowMonitorPol','infraRsL2PortAuthPol','infraRsMcpIfPol','infraRsL2PortSecurityPol','infraRsCoppIfPol','infraRsSpanVDestGrp','infraRsLacpPol','infraRsCdpIfPol','infraRsQosPfcIfPol','infraRsQosSdIfPol','infraRsMonIfInfraPol','infraRsFcIfPol','infraRsQosIngressDppIfPol','infraRsQosEgressDppIfPol','infraRsL2IfPol','infraRsStpIfPol','infraRsAttEntP','infraRsL2InstPol']
    ...
    if state == 'present':
        ...
        aci.get_diff(aci_class='infraAccBndlGrp')
